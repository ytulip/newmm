...
(function(root, factory) {
	...
	MobileSelectDate.prototype = {
		init: function(settings) {
            ...
            if(this.min.getFullYear() > this.value[0] || this.max.getFullYear()< this.value[0] ){
                this.value = [0, 0, 0];
                this.text = ['', '', ''];
                this.oldvalue = this.value.concat([]);
                this.oldtext = this.text.concat([]);
		...
		bindEvent: function() {
			...
			this.trigger.click(function(e) {
				...
				if( _this.settings.position == "bottom"){
					...
					if( _this.settings.hasOwnProperty('ignore_long') && _this.settings.ignore_long )
					{
                        var buttons=[{
                            'no': '取消'
                        },
				...
				$.confirm('<div class="ui-scroller-mask"><div id="' + _this.id + '" class="ui-scroller"><div></div><div ></div><div></div><p></p></div></div>', buttons, function(t, c) {
					...
					if(t == "long"){
                        _this.cancel();
                        $('input[name="MaturityDate"]').val('9999-12-31');
                        var normaltime = _this.trigger;
                        var longtime = document.querySelector('#longtime');
                        // normaltime.style.cssText = 'display:none;';
						normaltime.hide();
                        longtime.style.cssText = '';
					}
					if (t == 'no') {
						_this.cancel();
					}
		...
		submit: function() {
			...
			if (this.trigger[0].nodeType == 1) {
				...
				if( _this.settings.hasOwnProperty('ignore_long') && _this.settings.ignore_long ){
                    ...
                    $('input[name="MaturityDate"]').val(this.trigger.val());
				}
